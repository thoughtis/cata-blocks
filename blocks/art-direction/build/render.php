...
if ( 'diptych' === $art_direction_layout && isset( $art_direction_images[0] ) ) {
	// Two portraits read as a pair of verticals (3/4); a mixed pairing keeps
	// the neutral 8/7 so a landscape panel isn't cropped into a tall box.
	$art_direction_featured_meta     = wp_get_attachment_metadata( (int) get_post_thumbnail_id( $art_direction_post ) );
	$art_direction_featured_portrait = ! empty( $art_direction_featured_meta['width'] )
		&& ! empty( $art_direction_featured_meta['height'] )
		&& (int) $art_direction_featured_meta['height'] > (int) $art_direction_featured_meta['width'];
	$art_direction_diptych_ratio     = ( $art_direction_featured_portrait && 'portrait' === $art_direction_images[0]['orientation'] ) ? '3/4' : '8/7';
	...
	$art_direction_media = sprintf(
		...
		get_the_post_thumbnail( $art_direction_post, 'large', $art_direction_image_attributes( $art_direction_diptych_ratio ) ),
		wp_get_attachment_image( $art_direction_images[0]['id'], 'large', false, $art_direction_image_attributes( $art_direction_diptych_ratio ) )
	);
