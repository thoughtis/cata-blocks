 ...
 * @return string
 */
function cata_globe_animation_render_callback( array $attributes, string $content ): string {
	try {
		return cata_globe_animation_render_block( $attributes, $content );
	} catch( Throwable $e ) {
 ...
 * @return string
 */
function cata_globe_animation_render_block( array $attributes, string $content ): string {
	$content = substr( $content, 0, strrpos($content, '</div>') );
	$globe_color = array_key_exists( 'globeColor', $attributes ) ? $attributes['globeColor'] : '#000';

	$content .= '
			<div class="wp-block-cata-globe-background__globe-svg" style="stroke: ' . $globe_color . '">
