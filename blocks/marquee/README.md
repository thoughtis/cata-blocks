...
The animation is disabled (and the duplicate text hidden) under `prefers-reduced-motion`; the editor stylesheet disables it so the title stays legible while editing. The callback is wrapped in try/catch — failures report via the Query Monitor `qm/debug` action and fall back to the unmodified content.
